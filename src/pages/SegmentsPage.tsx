...
export default function SegmentsPage() {
  ...
  const handleFilterChange = (
    ...
    console.log('handleFilterChange - New Selected Attributes:', newSelectedAttributes);
  ...
  const handleUpdateSegment = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:3001/segments/${currentSegment.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleDeleteSegment = async (segmentId: string) => {
    try {
      const response = await fetch(`http://localhost:3001/segments/${segmentId}`, {
        method: 'DELETE',
      });
      ...
      const updatedSegments = savedSegments.filter((segment) => segment.id !== segmentId);
      setSavedSegments(updatedSegments);
      setCurrentSegment(null);
    } catch (error) {
