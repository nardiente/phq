...
export const GetCodeModal = ({ isOpen, onClose, widgetKey }: GetCodeModalProps) => {
  const [showCopied, setShowCopied] = useState(false);

  if (!isOpen) return null;

  const code = `<!-- Start of ProductHQ code snippet -->
    ...
    <>
      ...
      <div className="bg-white rounded-lg w-[600px] p-6 fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 border border-gray-200 z-[101]">
        <div className="flex justify-between items-center mb-4">
          ...
          <button onClick={onClose} className="text-gray-400 text-3xl font-light">×</button>
        </div>
        ...
        <pre className="bg-gray-800 text-white p-4 font-mono text-sm mb-2 w-full whitespace-pre-wrap break-all">{code}</pre>
        ...
        <p className="text-gray-600">
          Read the <a href="#" className="text-[#5a00cd]">Setup Guide</a> for more information or download the HTML example.
        </p>
      </div>
    </>
  );
