...
export const TabLauncher = ({ config, onClick }: TabLauncherProps) => {
  ...
  const isLeft = config.appearance?.position === 'Left';
      ...
      <div className="relative">
        <button
          ...
          className={`flex items-center gap-2 px-4 py-3 text-sm font-medium shadow-lg ${
            isLeft ? 'rounded-r-lg -translate-x-1' : 'rounded-l-lg translate-x-1'
          }`}
          style={{
        ...
        {config.notificationType === 'Count' && (
          <span className={`absolute -top-2 ${isLeft ? '-right-2' : '-left-2'} px-1.5 py-0.5 text-xs bg-red-500 text-white rounded-full`}>3</span>
        )}
        {config.notificationType === 'Dot' && (
          <span className={`absolute -top-2 ${isLeft ? '-right-2' : '-left-2'} min-w-[20px] min-h-[20px] bg-red-500 rounded-full`} />
        )}
      </div>
    </div>
  );
